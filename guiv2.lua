--[[
    ╔══════════════════════════════════════════╗
    ║          KICK MASTER PRO v3.2            ║
    ║  Auto-kick + God Mode + Kick All         ║
    ...
    Controls:
      ...
      • Drag shield speed slider
      • Set hits per target for Kick All
--]]
...
local Config = {
    ...
    Theme = {
        ...
        OrangeHover   = Color3.fromRGB(255, 185, 60),
        OrangeDim     = Color3.fromRGB(180, 100, 10),
...
local State = {
    ...
    hitsPerTarget  = Config.HitsDefault,
    kicks          = 0,
    shieldFires    = 0,
    dragging       = false,
...
local verBadge = label{
    ...
    Text = "v3.2", TextSize = 9, Font = Enum.Font.GothamBold,
...
-- ======== SEPARATOR 1 ========
Instance.new("Frame", statusPanel).Size = UDim2.new(1, -20, 0, 1)
local sep1 = statusPanel:FindFirstChildWhichIsA("Frame", true)
-- Actually let me just create them properly
...
sliderHitbox.MouseEnter:Connect(function()
    if not State.sliderDrag then tw(fillGlow, {BackgroundTransparency = 0.5}, 0.12) end
end)
sliderHitbox.MouseLeave:Connect(function()
    if not State.sliderDrag then tw(fillGlow, {BackgroundTransparency = 0.7}, 0.12) end
end)
...
playerList.Size = UDim2.new(1, 0, 1, -284)
...
bottom.Size = UDim2.new(1, 0, 0, 128)
bottom.Position = UDim2.new(0, 0, 1, -128)
bottom.BackgroundTransparency = 1; bottom.Parent = content

-- ======== KICK ALL CONFIG ROW ========
label{
    Size = UDim2.new(0, 100, 0, 20), Position = UDim2.new(0, 0, 0, 0),
    Text = "⚡ KICK ALL", TextSize = 10, Font = Enum.Font.GothamBold,
    TextColor3 = T.Orange, TextXAlignment = Enum.TextXAlignment.Left,
    Parent = bottom,
}

label{
    Size = UDim2.new(0, 60, 0, 20), Position = UDim2.new(1, -100, 0, 0),
    Text = "Hits/target:", TextSize = 9, Font = Enum.Font.Gotham,
    TextColor3 = T.TextSub, TextXAlignment = Enum.TextXAlignment.Right,
    Parent = bottom,
}

local hitsInput = Instance.new("TextBox")
hitsInput.Size = UDim2.new(0, 34, 0, 20)
hitsInput.Position = UDim2.new(1, -34, 0, 0)
hitsInput.BackgroundColor3 = T.SurfaceLight
hitsInput.Text = tostring(Config.HitsDefault)
hitsInput.PlaceholderText = "5"
hitsInput.TextSize = 11; hitsInput.Font = Enum.Font.GothamBold
hitsInput.TextColor3 = T.Orange; hitsInput.BorderSizePixel = 0
hitsInput.ClearTextOnFocus = false
hitsInput.Parent = bottom
addCorner(hitsInput, 5); addStroke(hitsInput, T.Border)

-- ======== KICK ALL BUTTON ========
local kickAllBtn = Instance.new("TextButton")
kickAllBtn.Size = UDim2.new(1, 0, 0, 40)
kickAllBtn.Position = UDim2.new(0, 0, 0, 24)
kickAllBtn.BackgroundColor3 = T.Orange
kickAllBtn.Text = "⚡   KICK ALL PLAYERS"
kickAllBtn.TextSize = 13; kickAllBtn.Font = Enum.Font.GothamBold
kickAllBtn.TextColor3 = T.Text; kickAllBtn.BorderSizePixel = 0
kickAllBtn.AutoButtonColor = false; kickAllBtn.ClipsDescendants = true
kickAllBtn.Parent = bottom; addCorner(kickAllBtn, 8)

-- progress bar inside kick all button
local kickAllProgressBar = Instance.new("Frame")
kickAllProgressBar.Name = "ProgressBar"
kickAllProgressBar.Size = UDim2.new(0, 0, 1, 0)
kickAllProgressBar.Position = UDim2.new(0, 0, 0, 0)
kickAllProgressBar.BackgroundColor3 = T.OrangeDim
kickAllProgressBar.BackgroundTransparency = 0.3
kickAllProgressBar.BorderSizePixel = 0
kickAllProgressBar.ZIndex = 0
kickAllProgressBar.Parent = kickAllBtn
addCorner(kickAllProgressBar, 8)

-- kick all button text sits above progress bar
kickAllBtn.ZIndex = 2

local kickAllBtnColors = {normal = T.Orange, hover = T.OrangeHover}

kickAllBtn.MouseEnter:Connect(function()
    if not State.kickAllActive then
        tw(kickAllBtn, {BackgroundColor3 = kickAllBtnColors.hover}, 0.1)
    end
end)
kickAllBtn.MouseLeave:Connect(function()
    tw(kickAllBtn, {BackgroundColor3 = kickAllBtnColors.normal}, 0.1)
end)

-- ======== AUTO KICK + GOD MODE BUTTONS ========
local kickBtn = Instance.new("TextButton")
kickBtn.Size = UDim2.new(0.485, 0, 0, 38)
kickBtn.Position = UDim2.new(0, 0, 0, 68)
...
shieldBtn.Size = UDim2.new(0.485, 0, 0, 38)
shieldBtn.Position = UDim2.new(0.515, 0, 0, 68)
...
-- KICK ALL ENGINE
---------------------------------------------------------------
local function resetKickAllBtn()
    State.kickAllActive = false
    kickAllBtn.Text = "⚡   KICK ALL PLAYERS"
    kickAllBtnColors.normal = T.Orange
    kickAllBtnColors.hover  = T.OrangeHover
    kickAllBtn.BackgroundColor3 = T.Orange
    tw(kickAllProgressBar, {Size = UDim2.new(0, 0, 1, 0)}, 0.3)
end

local function executeKickAll()
    if State.kickAllActive then
        -- Cancel
        State.kickAllActive = false
        notify("Kick All cancelled", T.Yellow, 2)
        return
    end

    State.kickAllActive = true
    kickAllBtn.Text = "■   CANCEL KICK ALL"
    kickAllBtnColors.normal = T.Red
    kickAllBtnColors.hover  = T.RedHover
    kickAllBtn.BackgroundColor3 = T.Red
    kickAllProgressBar.Size = UDim2.new(0, 0, 1, 0)
    ...
    task.spawn(function()
        ...
        if #targets == 0 then
            notify("No targets found", T.Yellow, 2)
            resetKickAllBtn()
            return
        end
        ...
        if not myRoot then
            notify("No character — cannot teleport", T.Red, 2.5)
            resetKickAllBtn()
            return
        end
        ...
        -- Auto-equip kick tool if in backpack
        pcall(function()
            local tool = LocalPlayer.Backpack:FindFirstChild("Kick")
            if tool then
        ...
        notify("Kick All started — " .. total .. " targets", T.Orange, 2.5)

        for i, target in ipairs(targets) do
            if not State.kickAllActive then break end

            -- Verify our character still exists
            myChar = LocalPlayer.Character
            myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
            if not myRoot then
                notify("Character lost — aborting", T.Red, 2.5)
                break
            end
            ...
            if targetRoot then
                -- Update button text
                kickAllBtn.Text = string.format("■  KICKING %d/%d — %s", i, total, target.DisplayName)
                ...
                task.wait(0.1) -- let server register new position

                -- Fire kick remote multiple times
                for h = 1, State.hitsPerTarget do
                    if not State.kickAllActive then break end
                    local remote = Config.KickRemotePath()
                    if remote then
                        local ok = pcall(function() remote:FireServer() end)
                ...
                -- Target has no character, skip
                completed += 1
            end

            -- Update progress bar
            local progress = completed / total
            tw(kickAllProgressBar, {Size = UDim2.new(progress, 0, 1, 0)}, 0.15)

            task.wait(0.15) -- delay between targets
        end
        ...
        local wasCompleted = State.kickAllActive

        -- Flash progress bar green on success
        if wasCompleted then
            tw(kickAllProgressBar, {BackgroundColor3 = T.Green}, 0.2)
            notify(
                "Kick All complete! " .. completed .. "/" .. total .. " players hit ("
                .. State.hitsPerTarget .. " hits each)",
                T.Green, 4
            )
        end
...
hitsInput.FocusLost:Connect(function()
    local v = tonumber(hitsInput.Text)
    if v and v >= Config.HitsMin and v <= Config.HitsMax and math.floor(v) == v then
        State.hitsPerTarget = v
        notify("Hits per target → " .. v, T.Orange, 2)
    else
        hitsInput.Text = tostring(State.hitsPerTarget)
        notify("Invalid hits (" .. Config.HitsMin .. " – " .. Config.HitsMax .. ", whole numbers)", T.Yellow, 2.5)
    end
end)
...
updateKickStats()
updateShieldStats()
notify("Kick Master Pro v3.2 loaded!", T.Accent, 4)
